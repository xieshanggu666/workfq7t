...
3. **成员角色（权限隔离）**：
   - **成员**：播种/浇水/施肥/除虫/收获、买卖交易、动物领养与养护、加工排产/退料/入库/队列重排、灌溉启停与地块水分设置、育种养护
   - **管理员**：以上全部 + 推进时间与**灾害结算**、建造/拆除灌溉防灾设施、发起杂交试验、邀请管理（可代任意成员取消/重排工单）
   - **场主**：以上全部 + 建筑升级、成员角色调整、**转让场主**、解散农场
   - 成员可随时退出（保留退出状态，可凭新邀请码回归）；场主须先转让或解散
   - **协作排产归属**：工单记录排产成员；成员只能取消/重排自己排产的工单，管理员/场主可代操作；旧存档无归属的工单视为全场共有
...
5. **加工（多人协作计划）**：加工坊按「生产工单」批量排产（面粉/番茄汁/奶酪等），每批耗时 1~2 个游戏天，
   队列串行推进、跨天自动完工；可随时取消工单退回未开工批次的原料，完工后一键入库；
   升级加工坊可扩容队列并解锁更多配方。
   - **作物与品种原料占用**：投料按批次逐项登记（基础作物 / 🧬杂交品种分开记录），
     排产即出库、按队列汇总为「🔒占用」；配方卡显示 **可用 = 库存 − 全队占用**，
     他人排产锁定的原料不会被重复排产；每张工单列出未开工批次锁定的品种明细
   - **取消退料**：仅未开工批次按原投料（含杂交品种）原样退回，正在加工的批次不退；
     已完工批次保留成品待入库
   - **队列重排**：尚未开工的工单可在排队窗口内上移/下移（交换计划顺序），
     正在加工的批次是不可越过的锚点；重放后机器自动按新顺序加工，投料仍归各自工单
   - **协作归属与实时状态**：工单显示排产成员头像/昵称与排队顺位，成员只能操作自己的工单（管理员可代操作），
     排产/取消/重排/入库经 SSE 实时推送，全农场在线端静默同步库存与队列
...
> 加工排产的纯逻辑在 `server/production-core.js`（队列重放/投料拆分/原料占用/重排计划，不依赖数据库），
> 可运行 `node --test server/irrigation-core.test.mjs server/production-core.test.mjs` 执行单元测试。
