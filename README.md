...
3. **成员角色（权限隔离）**：
   - **成员**：播种/浇水/施肥/除虫/收获、买卖交易、动物领养与养护、加工排产/队列重排/退料（仅自己的工单）/入库、灌溉启停与地块水分设置、育种养护
...
5. **加工（多人协作排产）**：加工坊按「生产工单」批量排产（面粉/番茄汁/奶酪等），每批耗时 1~2 个游戏天，
   队列串行推进、跨天自动完工；可随时取消工单退回未开工批次的原料，完工后一键入库；
   升级加工坊可扩容队列并解锁更多配方
   - **排产留痕**：每张工单记录排产人（昵称 + 在线状态点），队列顶部汇总各物品「🔒已锁原料」，
     配方行与背包区分「空闲可用库存 / 被队列占用」，杂交品种原料按品种逐项锁定与展示
   - **协作权限**：成员只能**取消自己排产**的工单，管理员/场主可取消任意工单；
     升级前无排产人的历史工单视为公共工单。排产/入库/重排所有成员均可操作
   - **队列重排**：尚未开工的等待工单可用 ▲▼ 与相邻等待工单交换顺序（seq 排序，重放自动重算排期）；
     已开工/加工中/已完工的工单钉死在时间线上，重排不会让机器倒回过去；取消工单后后续排队单自动提前
   - **取消退料**：按排产时逐批登记的实际投料原样退回（同本源杂交品种按各自品种退回，不会统一退成基础作物），
     只退未开工批次，加工中批次作废、已完工批次保留成品待入库
   - 所有操作走统一乐观版本 + SSE `mutation` 广播，多端排产/取消/重排后队列、占用与库存实时对齐
...
> `production_jobs` 额外带 `seq`（协作队列顺序，可重排）、`created_by/created_name`（排产人留痕与权限边界）、
> `inputs`（逐批实际投料明细，取消时按品种原样退料）；
...
> 协作排产（排产人留痕/原料占用/取消退料/队列重排/权限边界）的集成测试在
> `server/production.test.mjs`（用临时库自清理），或直接 `npm test` 跑全部。
